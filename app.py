...
AI_MODELS: Dict[str, str] = {
    "⚡ 穩定｜Gemini 2.5 Flash": "gemini-2.5-flash",
    "🧠 高品質｜Gemini 2.5 Pro": "gemini-2.5-pro",
}
...
def is_asset_protection_tag(tag: str) -> bool:
    """
    v20.5.2 + v22.3 + v26.7：判斷新聞台素材保護區。
    ...
    # [圖占版面xxx] — 例如 [圖占版面1/2] / [圖占版面右半]
    if re.search(r'[\[［]圖占版面[^\]］]*[\]］]?', t):
        return True
...
# =========================================================
# v24.2 華視編輯語：Headline Compiler / Slot Compiler
# =========================================================
def _headline_prefix_regex() -> str:
    return r"^(?:主標題?|大標題?|標題?|標)\s*[:=：＝]\s*"


def extract_headline_lines_v242(script: str) -> List[str]:
    """保留使用者在「大標:」後面手動換的行。

    規則：
    - 大標: 同行有字 → 視為第 1 行。
    - 大標: 下一行、下下一行 → 照原始換行保留。
    - 遇到空行、圖區、模組 tag、位置 tag 就停止。
    """
    if not script:
        return []
    ...
    for raw in lines:
        ...
        if not capture and re.match(_headline_prefix_regex(), line):
            capture = True
            after = re.sub(_headline_prefix_regex(), "", line).strip()
            if after:
                result.append(after)
            continue
        ...
        if capture:
            if not line:
                break
            if re.match(r"^(左|右|上|下|中|LEFT|RIGHT|TOP|BOTTOM)\s*[:=：]", line, flags=re.I):
                break
            if line.startswith(("(", "[")) and is_asset_protection_tag(line):
                break
            if re.match(r"^\([^)]*(?:圖|ROLL|roll|VCR|vcr|色塊|對話框|數據框|筆刷|蓋章|icon|箭頭)[^)]*\)", line):
                break
            if re.match(_headline_prefix_regex(), line):
                line = re.sub(_headline_prefix_regex(), "", line).strip()
            if line:
                result.append(line)
...
def _smart_split_two_line_headline(text: str) -> List[str]:
    """只有在使用者沒有手動給第二行時才備援拆句。"""
    text = " ".join((text or "").split()).strip()
    if not text:
        return ["", ""]

    # 優先從空白、驚嘆號、問號、逗號附近斷
    ideal = max(6, min(len(text) // 2, 18))
    candidates = []
    for idx, ch in enumerate(text):
        if ch in " 　，、!！?？；;／/｜|":
            candidates.append(idx + 1)
    if candidates:
        split = min(candidates, key=lambda x: abs(x - ideal))
    else:
        # 再從常見轉折／動詞前斷
        split = ideal
        for kw in ["爆", "控", "轟", "批", "曝", "疑", "週", "今", "明", "稱", "喊"]:
            pos = text.find(kw, max(4, ideal - 6))
            if pos > 2:
                split = pos
                break
    ...
    if not line2:
        ...
        line1, line2 = text[:half].strip(), text[half:].strip()
    return [line1, line2]


def compile_headline_text_v242(script: str, frame_type: str, fallback_title: str = "") -> str:
    """依版型鎖 headline 行數。

    - 標大框：永遠兩行；如果使用者手動換行，就 100% 保留前兩行。
    - 框訊：永遠一行；即使使用者換行，也合併成單行。
    - 記者說新聞：預設單行。
    """
    lines = extract_headline_lines_v242(script)
    if not lines and fallback_title:
        lines = [x.strip() for x in str(fallback_title).splitlines() if x.strip()]
    if not lines:
        return fallback_title.strip() if fallback_title else ""
    ...
    if ui_frame == "標大框":
        if len(lines) >= 2:
            return f"{lines[0]}\n{lines[1]}"
        auto_lines = _smart_split_two_line_headline(lines[0])
        return f"{auto_lines[0]}\n{auto_lines[1]}"

    # 框訊與記者說新聞一律單行
    return " ".join(lines).replace("\n", " ").strip()


def headline_constraint_block_v242(script: str, frame_type: str, title: str) -> str:
    ui_frame = normalize_frame_for_ui(frame_type)
    lines = [x.strip() for x in (title or "").splitlines() if x.strip()]

    if ui_frame == "標大框":
        line1 = lines[0] if len(lines) > 0 else ""
        line2 = lines[1] if len(lines) > 1 else ""
        return f"""
[HEADLINE COMPILER v24.2｜標大框兩行鎖死]
使用者已用換行決定標大框大標。這不是建議，是硬版面指令。
HEADLINE MODE: FORCE_TWO_LINES
HEADLINE LINE 1: {line1}
HEADLINE LINE 2: {line2}
RULES:
- 標大框大標必須永遠是兩行。
- 禁止把 LINE 1 和 LINE 2 合併成一行。
- 禁止重新斷句、禁止自動換行、禁止自動壓縮成單行。
- LINE 1 與 LINE 2 必須都在最上方 HEADLINE BAND 內。
- 大標是主視覺牆，不是普通文字框。
""".strip()
    ...
    if ui_frame == "框訊":
        one = " ".join(lines)
        return f"""
[HEADLINE COMPILER v24.2｜框訊一行鎖死]
HEADLINE MODE: FORCE_SINGLE_LINE
HEADLINE SINGLE LINE: {one}
RULES:
- 框訊大標必須永遠是一行。
- 禁止拆成兩行或多行。
- 如果太長，只能微調字級與字距，不可換行。
...
""".strip()
    ...
    return f"""
[HEADLINE COMPILER v24.2｜記者說新聞]
HEADLINE MODE: SINGLE_LINE_BY_DEFAULT
HEADLINE: {' '.join(lines)}
""".strip()
...
def slot_constraint_block_v242(script: str) -> str:
    ...
    return f"""
[SLOT LOCK v24.2｜禁止亂生圖框]
IMAGE / ROLL SLOTS EXPLICITLY DECLARED BY USER:
{lines_for(image_slots)}
QUOTE BUBBLES:
{lines_for(quote_slots)}
INFO CARDS:
...
def sanitize_script_for_image_model(script: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if stripped in ("##", "&&"):
            continue
        if re.match(r'^\(\d+(?:標大框|框訊|記者說新聞)[^)]*\)', stripped):
            continue
        if re.search(r'[-—]{2,}[""「」]?字變色', stripped):
            raw_line = re.sub(r'\s*[-—]{2,}[""「」]?字變色.*$', '', raw_line)
    ...
    # 0c. [日曆效果---xxx] / [日曆效果 xxx] → 轉成日曆視覺指令
    def _replace_calendar(m: re.Match) -> str:
    ...
    def _replace_paren(m: re.Match) -> str:
        ...
        if is_asset_protection_tag(raw):
            # 圖區語法直接從 content script 刪除
            # 位置和尺寸由 visual_zone_lines 統一處理，不在 script 裡重複出現
            return ""
...
def build_final_prompt_v18(
    ...
    # v24.2：依版型編譯 headline。標大框保留兩行；框訊鎖死單行。
...
{headline_constraint_block_v242(script, frame_type, parsed.title)}

{slot_constraint_block_v242(script)}
...
The following text has been pre-processed. All editor markup has been removed or converted.
...
# =========================================================
# v25.0 Imagen Prompt Translator：中英混雜導演指令 → 純英文視覺 prompt
# =========================================================
def translate_prompt_for_imagen(
    director_prompt: str,
    script: str,
    frame_type: str,
    api_key: str,
    ...
    """
    把 build_final_prompt_v18 產出的中英混雜導演規格，
    翻譯成 Gemini Imagen 真正聽得懂的純英文視覺 prompt。

    核心解決三個問題：
    1. 版型/結構跑掉：用座標比例取代語意描述（左右分欄 → x=0~50% / x=50~100%）
    2. AI 亂補內容：用 ABSOLUTE BLANK RECTANGLE 語法強化圖區指令
    3. 中文語意權重低：所有結構指令強制轉英文

    回傳 (translated_prompt, error_message)
    """
    if not configure_gemini(api_key):
        return None, "API 金鑰未設定"
    ...
    blank_zone_block = "\n".join(blank_zone_lines) if blank_zone_lines else "  - No explicit image zones. Do not invent any."
    ...
    layout_grid = layout_grid_map.get(frame_type, layout_grid_map["標大框"])

    translation_system_prompt = """You are a professional Taiwanese TV news CG art director with 20 years of broadcast experience.
Your task: Convert a Chinese/mixed-language broadcast CG layout specification into a clean, precise English image generation prompt for Gemini Imagen.

TRANSLATION RULES:
1. Output pure English only. No Chinese characters in the final prompt except for the approved Chinese news text (headlines, bullet points, names, quotes) which must stay in Traditional Chinese exactly as given.
2. Preserve ALL structural layout intent. Never simplify or omit layout zones.
3. For every image/ROLL/VCR zone: use the phrase "ABSOLUTE BLANK [WHITE/BLACK] RECTANGLE — 100% empty, no text, no icons, no labels, no fake photos, zero content inside, post-production real photo/footage will be inserted here"
4. Replace semantic layout descriptions with coordinate-based descriptions from the provided LAYOUT GRID.
5. CRITICAL — REMOVE ALL BRACKET TAGS: Delete every tag like [VISUAL DIRECTOR], [STYLE], [LAYOUT], [HEADLINE COMPILER], [TICKER SAFETY], [SLOT LOCK], [FRAME], [DETECTED LAYOUT INTENT], [SENIOR NEWS CG DESIGN BRIEF], [CONTENT SCRIPT], [DIRECTOR NOTES], [FINAL IMAGE RESTRICTIONS], [v19.6 DIRECTOR DECISION], [PROTECTED PHOTO ZONE] and any other [BRACKET TAG]. These will be rendered as visible text on the image if included. Convert their content into flowing visual description prose instead.
6. Maintain all prohibition rules word-for-word. These are critical constraints.
7. Do NOT add new facts, captions, or news content.
8. Output only the translated prompt. No explanations, no preamble.

CRITICAL — HEADLINE LINE BREAK RULES:
...
- State each person's card explicitly: "Person A card: [name] — [exact quote]", "Person B card: [name] — [exact quote]"
- Person silhouette icons are plain flat icons. They are NOT real photos and do NOT require a blank protected zone.

CRITICAL — EFFECT TAG RULES:
- The tag (章) in the original script is NOT a stamp/seal effect. It is just a bullet marker. Render it as a plain round bullet point or circular label, NOT as a wax seal or stamp graphic.
- Only render stamp/seal graphics if the original script explicitly says #蓋章, (蓋章效果), or [蓋章效果].
- Explosion/burst shapes like (爆炸效果) should render as a spiky burst shape background behind the text, NOT as a real explosion image."""

    translation_user_prompt = f"""ORIGINAL DIRECTOR PROMPT (Chinese/mixed):
---
{director_prompt[:3000]}
---

COORDINATE LAYOUT GRID TO USE:
{layout_grid}

BLANK ZONE TRANSLATIONS (use these exact phrases):
{blank_zone_block}

Please translate this into a clean English Gemini Imagen prompt following all rules above.
- Remove ALL [BRACKET TAGS] and convert their content to flowing prose.
- Keep all Chinese news text (headlines, bullet points, names, quotes) exactly as-is in Traditional Chinese.
- Use coordinate-based layout descriptions from the grid above.
- Use "ABSOLUTE BLANK RECTANGLE" phrasing for every image zone.
- State each headline line separately and forbid merging.
- State each person's dialogue card separately to prevent quote duplication.
- Treat (章) as a plain bullet marker, not a stamp effect."""
...
SELF-ADDED DECORATION PROHIBITION — these are NEVER allowed unless explicitly written in the user script:
- Yellow-black diagonal caution/warning tape strips (黃黑警戒條)
- Red circles or highlight circles drawn over image zones
- Annotation arrows or pointer graphics
- Crime scene tape or barrier tape patterns
- Broken glass or crack overlay effects
- Blood splatter or violence graphic overlays
- Any decorative border, frame, or overlay not specified by the user
...
[HEADLINE BAND ABSOLUTE LOCK]
The headline band is at the very top of the canvas.
標大框: top 40-48% of canvas (y=0 to approximately y=430).
框訊: top 15-18% of canvas (y=0 to approximately y=180).
...
- Cards, color blocks, information modules
- Icons of any kind (location pins, calendar icons, person icons)
- Speech bubbles, dialogue boxes
- Stamps, brush effects
- Any non-headline element whatsoever
...
def _parse_speaker_cards(script: str) -> List[Dict[str, Any]]:
    """
    v26.1：偵測稿件中的說話者卡片格式。
    支援：說話者名稱: / 說話者名稱： / 說話者名稱(假臉): 等
    回傳 list of dict: speaker, lines, has_icon, has_bubble
    """
    cards: List[Dict[str, Any]] = []
    lines = [l.strip() for l in script.splitlines()]
    i = 0
    COLUMN_KW = {"左", "右", "中", "上", "下", "左邊", "右邊", "中間", "左上", "左下", "右上", "右下"}
    HEADLINE_KW = {"大標", "主標", "標", "大標題", "主標題", "標題"}
    ...
    while i < len(lines):
        line = lines[i]
        if not line or re.fullmatch(r'[-—=]{3,}', line) or is_asset_protection_tag(line):
            i += 1
            continue
        m = SPEAKER_PATTERN.match(line)
        if m:
            raw_name = m.group(1).strip()
            if raw_name in COLUMN_KW or raw_name in HEADLINE_KW or len(raw_name) < 2:
                i += 1
                continue
            has_icon = bool(re.search(r'\(假臉\)', line))
            has_bubble = bool(re.search(r'\(對話框?\)', line))
            speaker_name = re.sub(r'\([^)]*\)', '', raw_name).strip()
            content_lines = []
            i += 1
            while i < len(lines):
                next_line = lines[i]
                if not next_line or re.fullmatch(r'[-—=]{3,}', next_line):
                    break
                if SPEAKER_PATTERN.match(next_line):
                    break
                if is_asset_protection_tag(next_line):
                    break
...
def build_visual_token_compiler_block(script: str, frame_type: str, headline_mode: str) -> str:
    ...
    # 先解析說話者卡片，用來排除重複行
    speaker_cards = _parse_speaker_cards(script)
    _speaker_lines: set = set()
    for card in speaker_cards:
        # 說話者名稱行和內容行都加入排除集合
        _speaker_lines.add(card["speaker"])
        _speaker_lines.add(card["speaker"] + "：")
        _speaker_lines.add(card["speaker"] + ":")
        for sl in card["lines"]:
            _speaker_lines.add(sl.strip())
    ...
    for block in blocks:
        ...
        for line in block:
            ...
            if re.fullmatch(r"[-—=]{3,}", stripped):
                continue
            # v26.4：說話者卡片的行不放進普通文字區塊
            cleaned_check = _clean_visual_text(stripped)
            if cleaned_check in _speaker_lines or stripped in _speaker_lines:
                continue
            # 說話者名稱行（冒號結尾）也排除
            if re.match(r'^.{1,20}[：:]\s*$', stripped) and not stripped.startswith(("左", "右", "中", "上", "下")):
                continue
    ...
    for tag in roll_tags:
        if roll_position:
            ...
            if frame_type == "標大框" or frame_type.startswith("框訊") or frame_type == "框訊":
                pos_text = "on the right or center-right side of the canvas, "
            else:
        ...
        visual_zone_lines.append(
            ...
            f"Width: 35-45% of canvas. Interior: 100% blank black/dark, no text, no icon, no label."
        )
    ...
    if not visual_zone_lines:
        if frame_type != "記者說新聞":
            visual_zone_lines.append(
                ...
                "AI decides 4:5 or 4:3 ratio. Interior: 100% blank."
            )
        else:
            visual_zone_lines.append("- No image zone specified. Keep content area clean.")
...
def build_cg_preview_html(script: str, frame_type: str, headline_mode: str, reporter_subtype_override: str = "", conclusion: Dict[str, str] | None = None) -> str:
    ...
    parsed = parse_user_script(script)
    parsed.title = compile_headline_text_v242(script, frame_type, parsed.title)
    title_lines = [html.escape(x.strip()) for x in (parsed.title or "主標題").splitlines() if x.strip()]
    if normalize_frame_for_ui(frame_type) == "標大框" and len(title_lines) >= 2:
        title = f"<span>{title_lines[0]}</span><span>{title_lines[1]}</span>"
    else:
        title = html.escape(" ".join((parsed.title or "主標題").splitlines()))
    tags = parsed.image_tags or (["[圖-右主]"] if frame_type in ["標大框", "框訊・流程關係"] else [])
    safe_tags = [html.escape(tag) for tag in tags]
    mode_class = "one-line" if normalize_frame_for_ui(frame_type) != "標大框" else "two-line"
...
.headline.two-line {{ font-size:38px; display:flex; flex-direction:column; align-items:flex-start; justify-content:center; gap:2px; }}
.headline.two-line span {{ display:block; line-height:1.02; }}
...
# =========================================================
# v24.3 草稿模式：手繪稿 → Scene Graph → Layout Constraints
# =========================================================
def build_draft_layout_analysis_prompt() -> str:
    return """
你是一位資深台灣電視新聞 CG 導播與版型編譯器。

任務：請分析使用者上傳的「手繪草稿 / 紙本草圖 / iPad 草圖」，不要直接生圖。
你要把草稿轉成可供新聞 CG 排版系統使用的 scene graph 與 layout constraints。

請特別辨識：
1. 大標區 headline_band：位置、比例、是否需要一行或兩行。
2. 主 ROLL / VCR / 黑框區：位置、大小、是否為主視覺，必須保護不被文字壓到。
3. 圖區 / 人物區 / 文件區：哪裡要留給後製真實素材。
4. 資訊卡 / 色塊 / 對話框：分組、相對位置、資訊流向。
5. 箭頭 / flow：誰指向誰、因果或攻防方向。
6. 版型類型：標大框、框訊、記者說新聞，若不確定請選最接近。
7. 密度：低 / 中 / 高。

請輸出 JSON only，不要加解釋文字。格式如下：
{
  "draft_mode_version": "v24.3",
  "frame_type_guess": "標大框 或 框訊 或 記者說新聞",
  "layout_intent": "一句話說明這張草圖的版面意圖",
  "headline": {
    "region": "top",
    "line_policy": "標大框=force_two_lines; 框訊=force_single_line; 記者說新聞=single_or_two",
    "height_ratio": 0.30,
    "dominance": "max"
  },
  "roll_zones": [
    {
      "id": "main_roll",
      "region": "right / center / center_right / left / bottom",
      "size_priority": "max / high / medium",
      "protected": true,
      "visual_fill": "black_video_window"
    }
  ],
  "image_zones": [
    {
      "id": "image_1",
      "region": "left / right / bottom / top_left / top_right",
      "protected": true,
      "purpose": "person_photo / document / scene_photo / evidence / unknown"
    }
  ],
  "card_groups": [
    {
      "id": "card_group_1",
      "region": "left / right / bottom / center_left / center_right",
      "type": "quote / fact_card / timeline / data / relation / warning",
      "density": "low / medium / high"
    }
  ],
  "flow": [
    {
      "from": "node_id_or_region",
      "to": "node_id_or_region",
      "direction": "left_to_right / right_to_left / top_to_bottom / bottom_to_top",
      "meaning": "attack / cause_effect / timeline / comparison / evidence"
    }
  ],
  "hard_constraints": [
    "用條列列出不能違反的空間規則"
  ],
  "render_notes": "給後續 Gemini / Nano Banana 生圖用的簡短導演備註"
}
""".strip()


def analyze_draft_layout_with_gemini(uploaded_file: Any, api_key: str, model_name: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    """把手繪稿辨識成 scene graph。這一步只理解 layout，不直接生圖。"""
    if not configure_gemini(api_key):
        return None, "Gemini API 尚未設定"
    if Image is None:
        return None, "尚未安裝 Pillow，請在 requirements.txt 加入 pillow"
    ...
    try:
        ...
        model = genai.GenerativeModel(model_name=model_name)
        response = model.generate_content(
            [build_draft_layout_analysis_prompt(), img],
            generation_config=genai.types.GenerationConfig(temperature=0.05),
        )
...
def draft_scene_graph_to_director_notes(scene_graph: Dict[str, Any]) -> str:
    """把草稿辨識結果轉成可併入 final prompt 的 layout constraints。"""
    if not scene_graph:
        return ""

    pretty = json.dumps(scene_graph, ensure_ascii=False, indent=2)
    frame_guess = scene_graph.get("frame_type_guess", "")
    layout_intent = scene_graph.get("layout_intent", "")
    render_notes = scene_graph.get("render_notes", "")

    return f"""
[DRAFT MODE v24.3｜HAND-DRAWN LAYOUT CONSTRAINTS]
The user provided a hand-drawn draft. Treat it as spatial layout intent, not as final artwork.
Do NOT copy pencil strokes. Do NOT render sketch lines. Use it only to preserve composition.

Frame guess from sketch: {frame_guess}
Layout intent: {layout_intent}
Render notes: {render_notes}

Scene graph extracted from sketch:
{pretty}

DRAFT MODE HARD RULES:
- Preserve the relative spatial relationship from the sketch.
- Keep headline in the detected headline band.
- Keep main ROLL/VCR zone as the dominant protected zone if detected.
- Do not move cards across major regions unless density requires minor adjustment.
- Do not overlap any text, arrows, cards, or decoration into protected image / ROLL zones.
- The sketch controls layout; AI may only beautify texture, color, typography style, lighting, and broadcast atmosphere.
- The sketch is a layout source, not a style reference image.
""".strip()


def render_scene_graph_summary(scene_graph: Dict[str, Any]) -> None:
    if not scene_graph:
        st.caption("尚未產生草稿 Scene Graph。")
        return
    st.markdown("### 🧠 草稿辨識結果")
    st.json(scene_graph)
    st.markdown("### 🔒 可導入的 Layout Constraints")
    st.code(draft_scene_graph_to_director_notes(scene_graph), language="markdown")


tab_ai, tab_draft, tab_prompt, tab_hole = st.tabs([
    "🤖 AI 拆稿",
    "✏️ 草稿模式",
    "🎬 v20.5 導演系統",
    "🖍️ 華視打洞機",
...
with tab_draft:
    st.subheader("✏️ v24.3 草稿模式｜手繪稿 → Scene Graph → 版型約束")
    st.caption("這一步只辨識空間版型，不直接生圖。辨識後可把 layout constraints 導入導演指令。")

    draft_col1, draft_col2 = st.columns([0.95, 1.05])
    with draft_col1:
        draft_file = st.file_uploader(
            "上傳手繪草稿 / iPad 草圖 / 白板照片",
            type=["png", "jpg", "jpeg", "webp"],
            key="draft_mode_upload",
        )
        if draft_file is not None:
            st.image(draft_file, caption="草稿預覽", use_container_width=True)

        if st.button("🔍 辨識草稿版型", type="primary"):
            if draft_file is None:
                st.warning("請先上傳草稿圖。")
            else:
                with st.spinner("正在辨識 headline / ROLL / 卡片 / 箭頭 / 資訊流…"):
                    scene_graph, err = analyze_draft_layout_with_gemini(
                        draft_file,
                        get_api_key(),
                        CURRENT_MODEL,
                    )
                if err:
                    st.error(f"草稿辨識失敗：{err}")
                else:
                    st.session_state["draft_scene_graph"] = scene_graph
                    st.session_state["draft_scene_graph_notes"] = draft_scene_graph_to_director_notes(scene_graph)
                    st.success("草稿已辨識完成，可導入導演系統。")

    with draft_col2:
        render_scene_graph_summary(st.session_state.get("draft_scene_graph", {}))
        if st.session_state.get("draft_scene_graph_notes"):
            if st.button("➡️ 導入導演系統備註", type="secondary"):
                st.session_state["draft_notes_enabled"] = True
                st.success("已導入。到『v20.5 導演系統』產生指令時會自動加入草稿版型約束。")

    st.markdown("### 使用方式")
    st.markdown(
        "1. 先上傳手繪稿。\n"
        "2. 按『辨識草稿版型』產生 Scene Graph。\n"
        "3. 按『導入導演系統備註』。\n"
        "4. 到導演系統貼新聞文字，系統會把草稿當成 layout constraints，而不是讓 AI 重新亂排。"
    )
...
with tab_prompt:
    ...
    with c2:
        ...
        if st.session_state.get("draft_notes_enabled") and st.session_state.get("draft_scene_graph_notes"):
            notes = notes + NL + NL + st.session_state["draft_scene_graph_notes"]
            st.success("✏️ 草稿模式約束已併入本次導演備註。")
    ...
    if st.session_state.get("imagen_translated_prompt"):
        ...
        with st.expander("📄 查看完整 Imagen 英文指令", expanded=True):
            st.code(_tp, language="markdown")

        # 防呆稽核：英文 prompt 裡不應出現中文結構指令
